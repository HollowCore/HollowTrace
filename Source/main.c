...
void ApplicationReady(void* context, HCApplicationRef application) {
    ...
    HCMenuSetTitle(aboutMenuItem, HCStringCreateWithCString("About Controls"));
    ...
    HCMenuSetTitle(quitMenuItem, HCStringCreateWithCString("Quit Controls"));
    ...
    HCWindowSetFrame(window, HCRectangleMake(HCPointMake(0.0, 0.0), HCSizeMake(1280.0, 720.0)));
    ...
    HCViewRef view2 = HCViewCreate();
    HCViewSetFrame(view2, HCRectangleMake(HCPointMake(400.0, 450.0), HCSizeMake(300.0, 200.0)));
    HCViewSetBackgroundColor(view2, HCColorBlue);
    HCViewAddChildView(HCWindowContentView(window), view2);
    ...
    HCRasterViewRef rasterView = HCRasterViewCreate();
    HCViewSetFrame((HCViewRef)rasterView, HCRectangleMake(HCPointMake(600.0, 100.0), HCSizeMake(500.0, 500.0)));
    HCViewSetBackgroundColor((HCViewRef)rasterView, HCColorGreen);
    HCRasterViewSetDrawCallback(rasterView, Draw, NULL);
    HCViewAddChildView(HCWindowContentView(window), (HCViewRef)rasterView);
...
void Draw(void* context, HCRasterViewRef view, HCRasterRef raster) {
    ...
    HCRasterFillQuad(raster, 0.0, 0.0, HCRasterWidth(raster), 0.0, HCRasterWidth(raster), HCRasterHeight(raster), 0.0, HCRasterHeight(raster), HCColorRed, HCColorGreen, HCColorBlue, HCColorWhite);
    HCRasterDrawPath(raster, dinosaur, HCColorBlack);
}
